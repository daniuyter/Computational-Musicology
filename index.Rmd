...
```{r}
least_danceable_song <- bigmusicmountain[which.min(bigmusicmountain$danceability), ]
least_danceable_song$track.name
```
...
```{r}
voodoochild <- get_tidy_audio_analysis("2AxCeJ6PSsBYiTckM0HLY7?si=79ee7af886dd419d")
...
themessage <- get_tidy_audio_analysis("5isA9icHWl2651hvfr1EOV?si=61dfe254355b4156")
themessage |>
  tempogram(window_size = 8, hop_size = 1, cyclic = TRUE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
An idea that came to me was if I could see which song is the least danceable v.s. the song that is most danceable, and make two tempograms out of them. Because beat/tempo consistency is a part of the danceability feature, I would expect the most danceable song to have a high beat consistency, whereas the least danceable song is expected to have a lot of tempo variation.

I have chosen to take the top 3 most and least danceable songs and to take the most interesting one out of each one.\
\
